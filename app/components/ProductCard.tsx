    ...
    return (
        <div className="product-card flex flex-col p-4 border rounded-lg shadow-md bg-white mb-2 h-full">
            ...
            <div className="mt-4 p-2 bg-gray-100 rounded-md">
                <h4 className="font-semibold">Technische Details</h4>
                <p>Preis: ca. {product.price} €</p>
                <p>Erscheinungsjahr: {product.year}</p>
                <p>Abmessungen: {product.dimensions}</p>
                <p>Hindernisüberwindung: {product.obstacleHeight}</p>
                <p>Arbeitszeit: {product.workingTime}</p>
                <p>Ladezeit: {product.chargingTime}</p>
            </div>
            <div className="mt-4 flex-grow">
            <Accordion>
                    <AccordionTab header="Funktionsumfang" className='font-semibold'>
                        <ul>
                            {featureEntries.map(([feature, available]) => (
                                available && (
                                    <li key={feature} className="flex items-center">
                                        <i className={`${PrimeIcons.CHECK_CIRCLE} text-green-500 font-bold mr-2`}></i>
                                        {feature}
                                    </li>
                                )
                            ))}
            ...
            {product.expertOpinion && <ExpertOpinion opinion={product.expertOpinion} />}
        </div>
    );
};
